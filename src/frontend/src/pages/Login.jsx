...
export default function Login() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      notify.success('Lien envoye a ' + forgotEmail)
      setShowForgot(false)
      setForgotEmail('')
    } catch {
      notify.error('Erreur. Reessayez.')
    } finally {
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        {!showForgot ? (
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div>
              ...
              <input
                ...
                className={"w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-green-500 " + (errors.email ? 'border-red-500 bg-red-50' : 'border-gray-300')}
            ...
            <div>
              ...
              <input
                ...
                className={"w-full px-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-green-500 " + (errors.password ? 'border-red-500 bg-red-50' : 'border-gray-300')}
                placeholder="........"
              />
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4" noValidate>
            ...
            <button type="submit" disabled={forgotLoading} className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 disabled:opacity-50">
              {forgotLoading ? 'Envoi...' : 'Envoyer le lien'}
            </button>
            <button type="button" onClick={() => { setShowForgot(false); setForgotEmail('') }} className="w-full text-sm text-gray-500 hover:underline">
